...
const mantenimiento = () => {
  ...
  const handleRegistrarMantenimiento = async () => {
    ...
    await fetch('http://localhost:4000/mantenimiento/registrar', {
      method: 'POST',
      headers: {
  ...
  const handleActualizarMantenimiento = async () => {
    ...
    await fetch(`http://localhost:4000/mantenimiento/actualizar/${mantenimientoActualizado.id_mantenimiento}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
        ...
        <table className="w-3/5 border-none shadow-sm overflow-hidden mx-auto">
          <thead className="bg-customBlue text-white">
            <tr>
              ...
              <th className="py-2 px-3 text-sm">ID Máquina</th>
          ...
          <tbody className="text-sm text-black bg-white divide-y divide-gray-200">
            {mantenimientos.map((mantenimiento) => (
              <tr key={mantenimiento.id_mantenimiento}>
                ...
                <td className="py-1 px-3">{mantenimiento.descripcion}</td>
                <td className="py-1 px-3">{mantenimiento.tipo_mantenimiento}</td>
                <td className="py-1 px-3">{mantenimiento.id_maquina}</td>
                <td className="py-1 px-3">{mantenimiento.id_usuario}</td>
        ...
        {selectedMantenimiento ? (
          <div className="bg-white w-96 p-6 rounded shadow-lg grid grid-cols-2 gap-4">
              ...
              <form>
                ...
                <div className="mb-4">
                  ...
                  <input
                    ...
                    className="w-full border border-gray-300 rounded px-3 py-2 mt-1"
                    placeholder="Nuevo Tipo Mantenimiento"
                  />
                </div>
              </form>
            </div>
            <div>
              {/* Columna 2 */}
              <div className="mb-4">
                <label htmlFor="id_maquina" className="block text-sm font-bold text-gray-700">ID Máquina:</label>
                <input
                  type="text"
                  id="id_maquina"
                  value={id_maquina}
